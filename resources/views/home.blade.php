                    ...
                    <tbody>
                        @foreach($latestNews as $news)
                            <tr>
                                <td>
                                    @if($news->image_path)
                                        <img src="{{ asset('storage/' . $news->image_path) }}" alt="News"
                                            class="img-circle img-size-32 mr-2">
                                    @else
                                        <img src="https://ui-avatars.com/api/?name={{ urlencode($news->title) }}&background=random"
                                            alt="News" class="img-circle img-size-32 mr-2">
                                    @endif
                                    <span class="fw-bold text-dark">{{ Str::limit($news->title, 40) }}</span>
                                    <br />
                                    <small class="text-muted ms-5">por
                                        {{ $news->author->pluck('name')->join(', ') ?: 'Redacción' }}</small>
                                </td>
            ...
            <div class="col-md-6">
                ...
                <div class="card bg-gradient-dark">
                    ...
                    <div class="card-body pt-0">
                        ...
                        <h4 class="text-center mt-3 mb-0 fw-bold">
                            {{ now()->locale('es')->isoFormat('D [de] MMMM [de] YYYY') }}</h4>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="col-lg-4">
        <div class="card shadow-sm border-0 bg-white">
            <div class="card-header bg-white border-bottom-0 pt-4">
                <h3 class="card-title fw-bold text-dark"><i class="fas fa-bolt text-warning me-2"></i> Acciones Rápidas
                </h3>
            </div>
            <div class="card-body">
                <div class="d-grid gap-3">
                    <a href="{{ route('admin.news.create') }}"
                        class="btn btn-outline-primary btn-lg text-start shadow-sm transition-hover">
                        <i class="fas fa-plus-circle me-3 fa-lg"></i> Redactar Noticia
                    </a>
                    <a href="{{ route('admin.banners.create') }}"
                        class="btn btn-outline-danger btn-lg text-start shadow-sm transition-hover">
                        <i class="fas fa-images me-3 fa-lg"></i> Subir Publicidad
                    </a>
                    <a href="{{ route('admin.vacancies.create') }}"
                        class="btn btn-outline-success btn-lg text-start shadow-sm transition-hover">
                        <i class="fas fa-briefcase me-3 fa-lg"></i> Publicar Vacante
                    </a>
                    <a href="{{ route('admin.audio_reports.create') }}"
                        class="btn btn-outline-info btn-lg text-start shadow-sm transition-hover">
        ...
        <div class="card card-widget widget-user-2 shadow-sm mt-4">
            <div class="widget-user-header bg-warning">
                <div class="widget-user-image">
                    <img class="img-circle elevation-2 bg-white p-1" src="{{ asset('storage/logotipo.png') }}"
                        alt="User Avatar">
                </div>
                ...
                <ul class="nav flex-column">
                    <li class="nav-item">
                        <a href="/" target="_blank" class="nav-link text-secondary fw-bold">
                            Ver Sitio Web <span class="float-right badge bg-primary"><i
